...
module.exports = function (context, options, payload) {
    ...
    if (!fsutils.dirExistsSync(resolveProjectPath(options.cwd, sourceDirName))) {
        var msg = "could not deploy functions because the " +
    ...
    if (!_.isEmpty(invalidNames)) {
        return utils.reject(invalidNames.join(", ") +
            " function name(s) must be a valid subdomain (lowercase letters, numbers and dashes)", { exit: 1 });
    }
    ...
    if (fsutils.fileExistsSync(packageJsonFile)) {
        try {
            ...
            if (!fsutils.fileExistsSync(indexJsFile)) {
                return utils.reject(path.relative(options.config.projectDir, indexJsFile) +
                    " does not exist, can't deploy Firebase Functions", { exit: 1 });
            }
        }
        catch (e) {
        ...
        return utils.reject("No npm package found in functions source directory. Please run 'npm init' inside " +
            sourceDirName, { exit: 1 });
    }
